    ...
    for (let i = 0; i < MAX_GAMEPADS; i++) {
      if (gamepads[i]) {
        ...
        for (let x = 0; x < gamepads[i].buttons.length; x++) {
          const { value } = gamepads[i].buttons[x]

          if (gp.buttons[x] !== undefined && gp.buttons[x] !== value) this.onButton(i, x, value)
        ...
        for (let x = 0; x < gamepads[i].axes.length; x++) {
          ...
          if (gp.axes[x] !== undefined && gp.axes[x] !== val) this.onAxis(i, x, val)
